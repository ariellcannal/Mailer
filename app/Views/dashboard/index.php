...
function updateChart() {
    ...
    $.ajax({
        url: '<?= base_url('dashboard/chart-data') ?>',
        data: { period: period },
        success: function(response) {
            // Extrai labels (datas)
            const labels = response.sends.map(item => item.date);
            
            // Extrai dados
            const sendsData = response.sends.map(item => item.count);
            const opensData = labels.map(date => {
                const found = response.opens.find(item => item.date === date);
                return found ? found.count : 0;
            });
            const clicksData = labels.map(date => {
                const found = response.clicks.find(item => item.date === date);
                return found ? found.count : 0;
            });
